...
public class JpqlBuilder {
	...
	public String getPageCountQueryString() {
		String hql=jpql.toString();
		hql=hql.replaceAll("(From)|(FRom)|(FROm)|(FROM)|(fRom)|(fROm)|(fROM)|(frOm)|(frOM)|(froM)", "from");
		String regex = "[^in\\s]+\\([^)]*([^(^)]*?)\\)";
		...
		// 清除fetch关键字
		String queryCount = StringUtils.replace(hql,
				" fetch ", " ");
		return queryCount;
	}
...
//		Query countQuery = this.createCountQuery(em);
		Query countQuery = this.createPageCountQuery(em);
...
//		long total =query.getResultList().size();
	...
	public Query createPageCountQuery(EntityManager em) {
		Query query = em.createQuery(getPageCountQueryString());
		for (Parameter parameter : parameters) {
			String name = parameter.getName();
			Object value = parameter.getValue();
			TemporalType temporalType = parameter.getTemporalType();
	...
	public static void main(String[] args) throws Exception {
		String hql="select new org.abc.dba.ca.User(select id from User where 1=1)From User fetch t where a.id=b.id and a.id=1 group by id";
		hql=hql.replaceAll("(From)|(FRom)|(FROm)|(FROM)|(fRom)|(fROm)|(fROM)|(frOm)|(frOM)|(froM)", "from");
		...
		hql="select count(*) "+hql.substring(from);
		// 清除fetch关键字
		String s="select new org.abc.dba.ca.User(select id from User where 1=1)From User fetch t where a.id=b.id and a.id=1 group by id";
		JpqlBuilder builder = new JpqlBuilder(s);
		System.out.println(builder.getPageCountQueryString());
	}
}
